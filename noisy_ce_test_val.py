...
def parse_option():
    ...
    opt.model_path = './save/noisy_CE/{}_models'.format(opt.dataset)  # 储存模型的位置
    opt.tb_path = './save/noisy_CE/{}_tensorboard'.format(opt.dataset)  # 储存训练的tb结果的位置
    opt.log_path = './save/noisy_CE/{}_logs'.format(opt.dataset)  # log记录结果
