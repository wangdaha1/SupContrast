...
def parse_option():
    ...
    # noisy dataset and model for linear probe
    ...
    opt.model_path = './save/noisy_LP/{}/{}_models'.format(opt.sslorsl, opt.dataset)  # 储存模型的位置
    opt.tb_path = './save/noisy_LP/{}/{}_tensorboard'.format(opt.sslorsl, opt.dataset)  # 储存训练的tb结果的位置
    opt.log_path = './save/noisy_LP/{}/{}_logs'.format(opt.sslorsl, opt.dataset)  # log记录结果
    # 因为是linear probe，直接用原来pretrain model的noisy dataset来做
...
def validate_cal(val_loader, model, classifier, criterion, opt):
    """validation"""
    model.eval()
    classifier.eval()
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # eval, here we just calculate the accuracy of classification of training data
        # so we use train_loader and set opt.split_per=1
        val_acc, label_precision_rate, clean_selection_num, noise_detection_rate, noise_detect_num = \
            validate_cal(train_loader, model, classifier, criterion, opt)  # train_loader
