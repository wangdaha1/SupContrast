...
# this is used for linear probe by noisy data and validate on clean test data
...
def parse_option():
    parser = argparse.ArgumentParser('argument for linear probe on noisy dataset')
    ...
    opt.model_path = './save/noisy_LP_2/{}/{}_models'.format(opt.sslorsl, opt.dataset)  # 储存模型的位置
    opt.tb_path = './save/noisy_LP_2/{}/{}_tensorboard'.format(opt.sslorsl, opt.dataset)  # 储存训练的tb结果的位置
    opt.log_path = './save/noisy_LP_2/{}/{}_logs'.format(opt.sslorsl, opt.dataset)  # log记录结果
    opt.figure_path = './save/noisy_LP_2/{}/{}_figures'.format(opt.sslorsl, opt.dataset)
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels, clean_labels) in enumerate(train_loader):
        ...
        # compute loss
        with torch.no_grad(): # 这样就可以不去计算gradient从而减少运算时间加快速度啦
            features = model.encoder(images)   # 丢掉projector的操作在这里啦
        output = classifier(features.detach())
        loss = criterion(output, labels)

        # losses and margins of clean and noisy samples
        loss_per_sample = torch.nn.CrossEntropyLoss(reduction='none')(output, labels)  # 计算每个sample的loss
        output_softmax = torch.nn.Softmax(dim=1)(output)

        margins = []  # 得写在这里 每个batch都是新的margins list
        for sample in range(bsz):  # 计算margin
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        train_loss, train_acc , clean_losses, noisy_losses, clean_margins, noisy_margins =\
            train(train_loader, model, classifier, criterion, optimizer, epoch, opt)
